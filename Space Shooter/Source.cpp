...
int bulletHit()
{
	for (int i = 0; i < 20; i++)
	{
		for (int j = 0; j < 4; j += 2)
		{
			if (bullets[i][j] != 0)
			{
				if (bullets[i][j] >= enemyY[0] && bullets[i][j + 1] <= enemyX[0] + 4)
			...
			{
				if (bullets[i][j + 1] >= enemyX[1] && bullets[i][j + 1] <= enemyY[1] + 4)
				{
					eraseBullet(i);
					resetEnemy(1);
					bullets[i][j] = 0;
...
int main()
{
	...
	do
	{
		...
		gotoxy(10, 10); cout << "2. Instuctions ";
